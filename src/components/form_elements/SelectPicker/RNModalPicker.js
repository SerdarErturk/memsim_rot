/* eslint-disable no-shadow */
/* eslint-disable react/prop-types */
// import libraries
import React, {useRef, useState} from 'react';
import {
  Text,
  View,
  Modal,
  Image,
  FlatList,
  Platform,
  StyleSheet,
  TouchableOpacity,
  I18nManager,
  TextInput,
} from 'react-native';
import { TouchableHighlight } from 'react-native-gesture-handler';
import { hp, wp } from "utils/dimension";
import FloatingLabel from './RNModalPickerInput';

// create a component
const RNModalPicker = (props) => {
  const {
    listTextStyle,
    data,
    status,
    labelText,
    selectedValue,
    disablePicker,
    selectedText,
    pickerStyle,
    selectedTextStyle,
    dropDownIconStyle,
    dropDownIcon,
    placeHolderText,
    placeHolderTextColor,
    changeAnimation,
    showPickerTitle,
    showSearchBar,
    searchBarStyle,
    pickerTitle,
    searchBarPlaceHolder,
    itemSeparatorStyle,
  } = props;

  const [dataSource, setData] = useState(data);
  const [modalVisible, setModalVisible] = useState(false);
  var [ isPress, setIsPress ] = React.useState(false);
  
  const setSelectedValue = (
    labelText,
    placeHolderText,
    placeHolderTextColor,
    defaultText,
    pickerStyle,
    selectedTextStyle,
    dropDownIconStyle,
    dropDownIcon,
  ) => {
    return (<View 
    style={disablePicker?
    {flexDirection:"row",borderWidth:hp(1),width:"104%",borderRadius:hp(11),backgroundColor: "#00000040",borderColor: "#39464E50"}:
    status=="danger"?   {flexDirection:"row",borderWidth:hp(1),width:"104%",borderRadius:hp(11),backgroundColor: "#EDF0F7",borderColor: "red"}:
    {flexDirection:"row",borderWidth:hp(1),width:"104%",borderRadius:hp(11),backgroundColor: "#EDF0F7",borderColor: "#39464E50"}}>
      <TouchableOpacity
        disabled={disablePicker}
        onPress={() => setModalVisible(true)}
        style={defaultText && !disablePicker?{width:"89%"}:{width:"101%"}}
        activeOpacity={0.7}>
        <View>
          <FloatingLabel
            container={pickerStyle}
         
            disabled={disablePicker}
            editable={false}
            style={[styles.selectedTextStyle]}
            dropDownIconStyle={[styles.dropDownIconStyle, dropDownIconStyle]}
            dropDownIcon={dropDownIcon}
            labelText={labelText}
            placeholder={placeHolderText}
            placeholderTextColor={placeHolderTextColor}
            value={defaultText}
          />
        </View>
      </TouchableOpacity>
      {defaultText && !disablePicker?<TouchableOpacity
      disabled={disablePicker}
      style={{width:"7%",alignItems:"center",justifyContent:"center",marginLeft:wp(8),marginTop:hp(2)}}
      onPress={() => {selectedValue(undefined,"");}}>
       <Image
      style={{ width: hp(25),
        height: hp(25),borderWidth:hp(1)}}
      resizeMode="contain"
      source={require('../../../images/botas/cross.webp')}
      />
      </TouchableOpacity>
       :null
      }
      
      </View>
    );
  };
  const searchFilter = (searchText, item) => {
    let filteredArray = [];
    if (searchText) {
      filteredArray = item.filter(function (value) {
        const itemData = value.title.toUpperCase();
        const textData = searchText.toUpperCase();
        return itemData.includes(textData);
      });
      setData([...filteredArray]);
    } else {
      setData([...data]);
    }
  };
  const listItemSeparator = (style) => {
    return <View style={style} />;
  };
 
  const renderItem = (item, index) => {
    return (
      <TouchableOpacity
        activeOpacity={0.3}
        style={styles.listItemTouchStyle}
        onPress={() => setSelectedIndex(index, item)}>
        <View style={styles.listItemStyle}>
          <Text style={[styles.listTextStyle]}>{item.title}</Text>
        </View>
      </TouchableOpacity>
    );
  };
  const setSelectedIndex = (index, item) => {
    selectedValue(index, item);
    setModalVisible(false);
  };
  return (

    <View style={styles.mainContainer}>
      {setSelectedValue(
        labelText,
        placeHolderText,
        placeHolderTextColor,
        selectedText,
        pickerStyle,
        selectedTextStyle,
        dropDownIconStyle,
        dropDownIcon,
      )}

      <Modal
        visible={modalVisible}
        transparent={true}
        onShow={() => setData(data)}
        animationType={changeAnimation}
        onRequestClose={() => {setModalVisible(false);}}
        >
        <View style={styles.container}>
          <View style={styles.listContainerStyle}>
            <View style={styles.pickerTitleStyle}>
              {showPickerTitle ? (
                <Text style={styles.pickerTitleTextStyle}> {pickerTitle}</Text>
              ) : null}

              <TouchableOpacity
                activeOpacity={0.7}
                onPress={() => setModalVisible(false)}>
                   <Image
                  resizeMode="contain"
                  style={styles.crossImageStyle}
                  source={require('../../../images/botas/cross.webp')}
                />
              </TouchableOpacity>
            </View>
            {showSearchBar ? (
              <View style={[styles.searchBarStyle, searchBarStyle]}>
                <TextInput
                  onChangeText={(text) => searchFilter(text, data)}
                  placeholder={searchBarPlaceHolder}
                  style={styles.textInputStyle}
                  underlineColorAndroid="transparent"
                  keyboardType="default"
                  returnKeyType={'done'}
                  blurOnSubmit={true}
                />
              </View>
            ) : null}

            <FlatList
              style={styles.flatListStyle}
              keyExtractor={(item) => item.value }
              showsVerticalScrollIndicator={false}
              // extraData={this.state}
              overScrollMode="never"
              ItemSeparatorComponent={() =>
                listItemSeparator(itemSeparatorStyle)
              }
              keyboardShouldPersistTaps="always"
              numColumns={1}
              data={dataSource}
              renderItem={({item, index}) => renderItem(item, index)}
            />
          </View>
        </View>
      </Modal>
    </View>
  );
};

// define your styles
RNModalPicker.defaultProps = {
  defaultValue: false,
  showSearchBar: false,
  showPickerTitle: false,
  disablePicker: false,
  dropDownIcon: require('../../../images/botas/down.webp'),
  changeAnimation: 'slide',
  //  placeHolderText: 'Please select value from picker',
  searchBarPlaceHolder: 'Arama...',
};
const styles = StyleSheet.create({
  mainContainer: {
    marginRight: wp(15),
    marginLeft:0,
    marginTop:wp(20),
    marginBottom:0
  },
  listTextStyle: {//modal liste elemanı
    color: '#000',
    marginVertical: 10,
    flex: 0.9,
    marginLeft: 20,
    marginHorizontal: 10,
    textAlign: 'left',
  },
  pickerTitleStyle: {//modal başlık
    flexDirection: 'row',
    width: '100%',
    alignItems: 'flex-end',
    justifyContent: 'flex-end',
    alignSelf: 'flex-end',
    marginTop:hp(4)
  },
  searchBarStyle: {// arama yeri
    marginBottom: 10,
    flexDirection: 'row',
    height: 40,
    shadowOpacity: 1.0,
    shadowRadius: 5,
    shadowOffset: {
      width: 1,
      height: 1,
    },
    backgroundColor: 'rgba(255,255,255,1)',
    shadowColor: '#d3d3d3',
    borderRadius: 10,
    elevation: 3,
    marginHorizontal: 10,
  },

  flatListStyle: {//modal içi liste
    maxHeight: '85%',
  },

  container: {//modal arka plan
    flex: 1,
    backgroundColor: 'rgba(0,0,0,0.4)',
    justifyContent: 'center',
  },
  listItemStyle: {//modal içi liste
    width: '100%',
    justifyContent: 'center',
  },
  textInputStyle: {//modal içindeki arama yeri
    color: 'black',
    paddingLeft: 15,
    marginTop: Platform.OS === 'ios' ? 10 : 0,
    marginBottom: Platform.OS === 'ios' ? 10 : 0,
    alignSelf: 'center',
    flex: 1,
    textAlign: I18nManager.isRTL ? 'right' : 'left',
  },
  crossImageStyle: {
    width: wp(25),
    height: hp(25),
    borderWidth:hp(2),
   marginBottom:hp(12),
   marginRight:wp(6)
 
  },
  selectedTextStyle: {
    height: hp(50),
    borderColor: 'gray',
    backgroundColor: 'transparent',
    justifyContent: 'center',
    width: '100%',
    color: 'black',
    marginTop: hp(4),  
    fontSize: hp(14),
    minHeight: hp(40),
    color: "#39464E",
    fontFamily: "Narin-Bold",
  },
  listContainerStyle: {
    alignSelf: 'center',
    width: '90%',
    borderRadius: 10,
    maxHeight: '80%',
    backgroundColor: 'white',
  },

  listItemTouchStyle: {
    justifyContent: 'center',
    flexDirection: 'row',
    flex: 1,
  },
  dropDownIconStyle: {
    width: 20,
    height: 20,
    left: -40,
    // marginTop: 20,
  },
  pickerTitleTextStyle: {
    fontSize: 18,
    flex: 1,
    paddingBottom: 10,
    marginLeft: wp(35),
    color: '#000',
    textAlign: 'center',
    
  },
});
export default RNModalPicker;
